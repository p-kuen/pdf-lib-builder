import {DrawTextOptions, PDFFont, PDFPageDrawTextOptions, TextAlignment, degrees, drawText} from '@patcher56/pdf-lib'
...
function alignX(text: string, x: number, font: PDFFont, fontSize: number, align?: TextAlignment) {
  if (!align) {
    return x
  }
...
function breakTextIntoLinesOfPage(
  builder: PDFDocumentBuilder,
  text: string,
  options?: Pick<PDFBuilderPageDrawTextOptions, 'wordBreaks' | 'maxWidth' | 'maxLines' | 'size'>
) {
  const wordBreaks = options?.wordBreaks || builder.doc.defaultWordBreaks
  const maxWidth = Math.min(
    options?.maxWidth || Infinity,
    builder.page.getWidth() - builder.x - builder.options.margins.right
  )

  const fontSize = options?.size || builder.fontSize
  const textWidth = (t: string) => builder.font.widthOfTextAtSize(t, fontSize)

  const textLines = breakTextIntoLines(
    text,
    wordBreaks,
    (lineNumber) => (lineNumber === 1 ? maxWidth : Math.min(maxWidth, builder.maxX - builder.options.margins.right)),
    textWidth
  )
